...
if LOG_FILE:
    from varen_py.common.file_logging import file_logger

    logger = file_logger
else:
    from varen_py.common.console_logging import console_logger

    logger = console_logger


varen = Varen (env=TESTNET, l1_address=TEST_L1_ADDRESS, l1_private_key=TEST_L1_PRIVATE_KEY)

recipient_address = ""
amount = Decimal(0)
logger.info(f"Transferring USDC from {TEST_L1_ADDRESS} to {recipient_address} amount {amount}")

asyncio.run(varen.account.transfer_on_l2(recipient_address, amount))
